...
**Runs on PRs but is NOT a merge gate** (i.e. not a required status context — read it, don't treat its red as a branch-protection block). That is not the same as "ignorable": the first entry below goes red until *you* act, and clearing it is the author's job:

- `ci-integrity.yml` → `Guard gating workflows from self-edits`. Fires on a PR **based on `main`** that touches `.github/workflows/**` or `.github/actions/**` — that trigger IS the scope, minus a deliberately empty `NON_GATING` list, so there is no allowlist to add a file to or remove one from. It goes red until you **declare** the change, and declaring it is your job, not an operator's:
  - Purely **additive** (a new job, a new workflow file, nothing existing altered): `gh pr edit <n> --add-label 'ci:gate-change=declared'`.
  - Changing an **existing** gate surface: `ci:gate-change=alters-a-gate` *and* a whole line in the PR body naming each altered item — `Gate-Change: <stem>#<job-key>` (or `Gate-Removal:`), using `<stem>#workflow` for a change outside `jobs:`.
  - A label does not re-trigger the workflow, so **re-run the job** after applying it.
  - A comment-only edit still trips it, because the scope is the changed-file list rather than the digest. Whether it needs the *stronger* label depends on where the comment is, and this catches people out: a YAML-level comment is dropped by the parser and alters no surface — but a **shell comment inside a `run: |` block** is part of that block scalar's value, so editing one moves a digest. (A trailing `# …` on a plain `run: cargo test` is stripped; the same text inside a `run: |` block is not.) The label is **per-PR, not per-edit**: if *any* file in the PR moves a surface, the whole PR needs `alters-a-gate` — this very bullet's PR is the example, most of its workflow edits moved nothing and it still did.
  - The token to write: `<stem>#<job-key>` for a job; `<stem>#workflow` for a change outside `jobs:`; and in a **composite action** under `.github/actions/**` there are no jobs at all, so it is always `<stem>#workflow`, with `<stem>` the action's *directory* name. When in doubt, read the guard's log — it prints the exact directive lines it wants.
