...
function Get-EffectProbeProblems {
    ...
    foreach ($surface in @($Surfaces)) {
        ...
        foreach ($actionId in @($ExpectedEffects.Keys)) {
            ...
            # A resolved "action" that is itself a COLLECTION is a caller
            # fault, and it must not be rendered as a serializer verdict.
            #
            # MEASURED (CI run 34762669548, windows-latest): the key check below
            # fired on every action it found, while `$_.id` matched on that same
            # object and a genuinely-removed action still reported `absent`.
            # Cause: `@($json | ConvertFrom-Json)` is not engine-portable.
            # Windows PowerShell 5.1 emits a JSON array as ONE Object[] item
            # instead of enumerating it, so `@(pipeline)` nests there and is
            # flat on pwsh 7. `$action` was then the Object[] itself, whose
            # properties are Length/Count/Rank -- no `effect`, and the branch
            # below announced a serializeComponent regression that had not
            # happened. The harness bug is fixed at its source in
            # tests/test-effect-probe.ps1; this guard makes the shape
            # self-announcing if any future caller reintroduces it, because a
            # confident wrong answer is worse than the defect it masks.
            if ($action -is [System.Array]) {
                $problems += "${surfaceName}: '$actionId' resolved to a COLLECTION, not an action object -- the caller passed a nested array; this is a harness fault, NOT a serializer regression"
                continue
            }
