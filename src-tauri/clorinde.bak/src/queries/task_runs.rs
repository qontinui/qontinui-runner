...
pub fn get_task_run() -> GetTaskRunStmt {
    GetTaskRunStmt(
        "SELECT id, task_name, prompt, task_type, status, sessions_count, max_sessions, error_message, auto_continue, execution_steps_json, log_sources_json, config_id, workflow_name, workflow_id, COALESCE(summary, ai_summary) as summary, ai_summary, goal_achieved, remaining_work, summary_generated_at, transition_history_json, workflow_type, workspace_id, triggered_by, parent_task_run_id, root_task_run_id, depth, bridge_id, result_data, COALESCE(is_reflection, false) as is_reflection, reflection_source_task_run_id, COALESCE(is_follow_up, false) as is_follow_up, follow_up_source_task_run_id, COALESCE(is_fixer, false) as is_fixer, fixer_source_task_run_id, COALESCE(is_meta_optimizer, false) as is_meta_optimizer, created_at, updated_at, completed_at FROM task_runs WHERE id = $1",
        None,
    )
}
impl GetTaskRunStmt {
...
pub fn get_recent_task_runs() -> GetRecentTaskRunsStmt {
    GetRecentTaskRunsStmt(
        "SELECT id, task_name, prompt, task_type, status, sessions_count, max_sessions, error_message, auto_continue, config_id, workflow_name, workflow_id, COALESCE(summary, ai_summary) as summary, ai_summary, goal_achieved, remaining_work, summary_generated_at, workspace_id, triggered_by, created_at, updated_at, completed_at FROM task_runs WHERE (workflow_type IS NULL OR workflow_type != 'chat') AND ($1::integer IS NULL OR runner_port IS NULL OR runner_port = $1) ORDER BY updated_at DESC LIMIT $2",
        None,
    )
}
...
pub fn get_running_task_runs() -> GetRunningTaskRunsStmt {
    GetRunningTaskRunsStmt(
        "SELECT id, task_name, prompt, task_type, status, sessions_count, max_sessions, error_message, auto_continue, config_id, workflow_name, workflow_id, workflow_type, workspace_id, triggered_by, parent_task_run_id, root_task_run_id, depth, bridge_id, COALESCE(is_reflection, false) as is_reflection, reflection_source_task_run_id, COALESCE(is_follow_up, false) as is_follow_up, follow_up_source_task_run_id, COALESCE(is_fixer, false) as is_fixer, fixer_source_task_run_id, COALESCE(is_meta_optimizer, false) as is_meta_optimizer, runner_port, created_at, updated_at, completed_at FROM task_runs WHERE status = 'running' AND ($1::integer IS NULL OR runner_port IS NULL OR runner_port = $1) ORDER BY created_at DESC",
        None,
    )
}
...
pub fn get_recent_task_runs_filtered() -> GetRecentTaskRunsFilteredStmt {
    GetRecentTaskRunsFilteredStmt(
        "SELECT id, task_name, prompt, task_type, status, sessions_count, max_sessions, error_message, auto_continue, config_id, workflow_name, workflow_id, COALESCE(summary, ai_summary) as summary, ai_summary, goal_achieved, remaining_work, summary_generated_at, workspace_id, triggered_by, created_at, updated_at, completed_at FROM task_runs WHERE ($1::text IS NULL OR workflow_type = $1) AND (workflow_type IS NULL OR workflow_type != 'chat') AND ($2::integer IS NULL OR runner_port IS NULL OR runner_port = $2) ORDER BY updated_at DESC LIMIT $3",
        None,
    )
}
