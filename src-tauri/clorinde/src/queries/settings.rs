...
pub struct CountSettingsStmt(&'static str, Option<tokio_postgres::Statement>);
pub fn count_settings() -> CountSettingsStmt {
    CountSettingsStmt("SELECT COUNT(*)::bigint AS count FROM settings", None)
}
impl CountSettingsStmt {
    pub async fn prepare<'a, C: GenericClient>(
        mut self,
        client: &'a C,
    ) -> Result<Self, tokio_postgres::Error> {
