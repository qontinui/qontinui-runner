...
UPDATE task_runs SET
    total_input_tokens = COALESCE((SELECT SUM(input_tokens) FROM phase_token_usage WHERE task_run_id = :task_run_id), 0)::bigint,
    total_output_tokens = COALESCE((SELECT SUM(output_tokens) FROM phase_token_usage WHERE task_run_id = :task_run_id), 0)::bigint,
    total_cost_cents = COALESCE((SELECT SUM(cost_cents) FROM phase_token_usage WHERE task_run_id = :task_run_id), 0)::bigint
