...
/// How a coord launch came out of the drain hold.
#[derive(Debug, PartialEq, Eq)]
enum LaunchHold {
    /// The authorization allows the spawn.
    Admitted(crate::agent_authorization::SpawnDecision),
    /// The registry refused it (a standing decision, not a drain deferral).
    Refused(crate::agent_authorization::SpawnDecision),
    /// Coord stopped the agent while it was held.
    Stopped,
}

/// Authorize a coord launch, HOLDING while the device drain defers it: wait
/// for `wait_allowed` (or `stop`), then authorize again. A drain deferral is
/// never returned — only an allow, a registry refusal, or a stop.
async fn hold_launch_for_drain<A, AF, W, WF>(
    mut authorize: A,
    mut wait_allowed: W,
...
where
    A: FnMut() -> AF,
    AF: std::future::Future<Output = crate::agent_authorization::SpawnDecision>,
    W: FnMut() -> WF,
    WF: std::future::Future<Output = ()>,
{
    loop {
        if stop.is_cancelled() {
            return LaunchHold::Stopped;
        }
        let authz = authorize().await;
        if !authz.is_deferred_by_drain() {
            return if authz.allows_spawn() {
                LaunchHold::Admitted(authz)
            } else {
                LaunchHold::Refused(authz)
            };
        }
        info!(
            "agent_runtime: coord launch held — {}",
            authz.reason().unwrap_or("deferred by the device drain")
        );
        tokio::select! {
            _ = stop.cancelled() => return LaunchHold::Stopped,
            _ = wait_allowed() => {}
        }
    }
}
...
fn spawn_run_task(payload: LaunchPayload) {
    ...
    tokio::spawn(async move {
        ...
        // A drain deferral is HELD, not reported as a failure: the launch waits
        // (stoppable by coord's stop request) and is re-authorized the moment
        // autonomous spawns may run. The boot read comes first so a launch
        // delivered at boot is not judged against the not-yet-read state.
        crate::coord_drain_state::await_boot_read(Duration::from_secs(15)).await;
        let hold = hold_launch_for_drain(
            || {
        ...
        .await;
        let authz = match hold {
            LaunchHold::Admitted(authz) | LaunchHold::Refused(authz) => authz,
            LaunchHold::Stopped => {
                info!(
...
enum ClaimOutcome {
    ...
    /// so the stamp coord receives says `device_drain:drained` vs
    /// `device_drain:unknown` from the DECISION rather than from a re-read of
    /// the live state, which can have moved on since the claim was folded.
    DeferUnclaimed(crate::coord_drain_state::DeferClass),
}
...
/// plus the anchors RESERVED by dispatches that passed the guard and have not
/// yet registered (or given up).
///
/// Both halves sit under ONE mutex on purpose. P3's "is this anchor taken?"
...
fn prune_dead_continuations(is_live: &dyn Fn(&str) -> bool) {
    ...
    // Only `live` is reaped: a pending anchor has no terminal to test and is
    // released by the dispatch that holds it (`AnchorReservation`).
    let reaped: Vec<ContinuationSession> = {
        let mut registry = lock_recover(continuation_sessions(), "continuation_sessions");
        dead.iter()
            .filter_map(|tid| registry.live.remove(tid))
            .collect()
    };
...
/// Who owns an anchor the guard found taken (the payload of
/// [`ContinuationGuard::DuplicateAnchor`]).
#[derive(Debug, PartialEq, Eq)]
enum AnchorHolder {
...
impl AnchorReservation {
    ...
    /// this anchor, so the guard must stop reporting it as taken.
    fn release(self) {
        drop(self);
    }
...
async fn claim_before_spawn(gate_id: uuid::Uuid, device_id: uuid::Uuid) -> SpawnDecision {
    let Some(base) = connected_coord_base() else {
        // No coord configured: there is no claim surface to consult, and a
        // runner with no coord URL is `NotEnrolled`, so no device drain can
        // apply either. Proceed quietly, as before — this is not a claim error.
        return SpawnDecision::Spawn;
    };
...
fn duplicate_anchor_stamp_reason(holder: &AnchorHolder) -> String {
    match holder {
        AnchorHolder::Live(existing_terminal_id) => {
            format!("duplicate_anchor:{existing_terminal_id}")
        }
        // A reservation has no terminal id; the detail names the holder's
        // kind instead so the class parser still finds a non-empty detail.
        AnchorHolder::Reserved => "duplicate_anchor:reserved".to_string(),
    }
}
...
fn drain_stamp_reason(class: crate::coord_drain_state::DeferClass) -> String {
    format!("device_drain:{}", class.label())
}
...
async fn run_gate_continuation_inner(
    ...
    match guard_verdict {
        ...
        ContinuationGuard::DuplicateAnchor(holder) => {
                ...
                AnchorHolder::Live(existing_terminal_id) => {
                    info!(
                        "agent_runtime: gate-continuation deduped by anchor_key={:?} — a live \
                         continuation session (terminal_id={existing_terminal_id}) already \
                         exists; skipping double-spawn",
                        payload.anchor_key
                    );
                    // Best-effort: bring the existing docked tab to focus so the
                    // operator sees the live continuation rather than nothing
                    // happening.
            ...
            // The holder of the anchor (the live session, or the dispatch about
            // to spawn one) owns its own claim+outcome; the deduped gate stays
            // pending on coord (its work IS that session) until cancelled or it
            // expires. Once the anchor's session exits — or the reservation is
            // released without a spawn — a re-delivery of this same gate_id must
            // be able to dispatch, which is what the release inside
            // `defer_continuation_unclaimed` buys.
    ...
    // has. `None` (worktree mode off / acquire declined) → the verified
    // fallback cwd, or a refusal when the repo has no checkout here.
    let intent = payload
        .anchor_key
        .as_deref()
...
async fn run_continuation_terminal(
    ...
    // The anchor reservation the guard left behind. Every early `Err` below
...
async fn acquire_continuation_workdir(
    ...
    //
    // **A continuation never runs where its repo is not** (plan
    // `2026-09-12-continuation-for-a-repo-outside-the-workspace-root-spawns-into-an-empty-directory`,
    // Phase 2). The root is only a sane cwd for a repo checked out UNDER it; a
    // tenant's own repo lives elsewhere, and landing it at the root (or, before
    // the root-first order, in a directory `<root>/<name>` that provisioning then
    // created) reported `spawned` for a session with no repo in reach. So the
    // repo's checkout is VERIFIED first, and a continuation with none is
    // refused here — before coord-mcp or command provisioning can create
...
async fn run_agent_subprocess(
    ...
    // Provision the named-subagent defs (embedded floor + checkout overlay),
    // fleet slash commands and fleet skills into the spawned worktree's cwd, so
...
/// session cwd — an agent worktree, a continuation or looping-agent workdir, a
/// terminal cwd — so `claude` can resolve the subagents a prompt references
...
pub(crate) fn provision_agent_definitions_from_root(
        ...
        .map_err(|e| anyhow::anyhow!("read agents dir {}: {e}", src_dir.display()))?
    {
        ...
        if tracked.should_skip(&dst, Path::new(name)) {
            info!(
                "agent_runtime: skipping {} — it is tracked by the enclosing git \
...
mod tests {
    ...
    /// The checkout overlay honours the tracked-file guard too: a cwd that is a
    /// repo tracking `.claude/agents/code-reviewer.md` keeps those bytes through
    /// BOTH layers (the embedded floor writes first, so surviving bytes prove it
    /// skipped too), while an untracked sibling is still overlaid.
    #[test]
    fn provision_agent_defs_skips_a_tracked_destination_in_both_layers() {
        ...
        let src = root
            ...
            .join("agents");
        std::fs::create_dir_all(&src).unwrap();
        std::fs::write(src.join("code-reviewer.md"), "# checkout reviewer").unwrap();
        std::fs::write(src.join("repo-auditor.md"), "# checkout auditor").unwrap();
        ...
        assert_eq!(
            ...
            "# the repo's own reviewer",
            "a tracked definition must survive both the floor and the overlay"
        );
        assert_eq!(report.expected, 2);
        assert_eq!(report.written, 1, "the untracked sibling is still overlaid");
    ...
    fn a_repo_with_no_verified_checkout_is_refused_not_rooted() {
        ...
        let resolve = |repo: &str| {
            Err::<std::path::PathBuf, _>(
                crate::agent_worktree::canonical_paths::CheckoutUnresolved {
                    repo: repo.to_string(),
                    tried: vec![(
                        std::path::PathBuf::from("D:/portofino-pizzeria/mobile"),
                        "owner-root convention: not a git checkout".to_string(),
                    )],
                    detail: None,
                },
            )
    ...
    fn a_held_worktree_claim_falls_through_without_waiting() {
        ...
        let conflict: Result<Option<u8>, AllocateError> =
            Err(AllocateError::ClaimConflict(ClaimConflict {
                kind: "worktree".to_string(),
                resource_key: "d:/portofino-pizzeria/mobile".to_string(),
    ...
    fn clear_continuation_registry() {
        ...
        // Both halves: a reservation left by a `Proceed` in one test must not
        // read as `DuplicateAnchor` in the next.
    ...
    /// Phase 4 of plan
    /// `2026-09-13-one-landed-pr-dispatches-its-follow-up-to-two-sessions-through-two-gate-anchors`:
    /// P3 reserves the anchor INSIDE its critical section, so two evaluations
    /// for one anchor with no registration between them — the burst window,
    /// where every task of one poll iteration used to see the pre-burst
    /// registry — cannot both `Proceed`. The second reads the reservation, and
    /// carries `Reserved` (no terminal exists yet to focus). Deleting the
    /// `pending_anchors.insert` in the guard fails this test.
    #[test]
    fn same_anchor_reservation_dedups_a_second_evaluation_before_registration() {
        let _env_lock = env_lock();
        let _restore = crate::test_env::EnvVarRestore::capture(CAP_ENV_KEYS);
        let _g = CONT_GUARD_LOCK.lock().unwrap_or_else(|p| p.into_inner());
        clear_continuation_registry();
    ...
    fn the_deferred_stamp_reasons_follow_the_class_detail_grammar() {
        // 1. Dedup: the class, then the terminal id that won the anchor — or
        //    `reserved` when the winner is a dispatch that has not spawned yet.
        assert_eq!(
            duplicate_anchor_stamp_reason(&AnchorHolder::Live("term-tid-1".to_string())),
            "duplicate_anchor:term-tid-1"
        );
        assert_eq!(
            duplicate_anchor_stamp_reason(&AnchorHolder::Reserved),
            "duplicate_anchor:reserved"
        );
        ...
        // The `<class>:<detail>` reasons share the grammar: exactly one leading
        // class, split on the first colon, and a non-empty detail after it.
        for reason in [
...
mod drain_claim_tests {
    ...
    fn drain_stamp_follows_the_class_detail_grammar() {
        ...
        assert_eq!(
            drain_stamp_reason(DeferClass::Drained),
            "device_drain:drained"
        );
        assert_eq!(
            drain_stamp_reason(DeferClass::Unknown),
            "device_drain:unknown"
        );
        // The stamp comes from the decision, so it names the class that
        // deferred even if the live state has moved on since.
        for state in states_that_pause() {
            let decision = apply_drain_to_claim(decide_spawn(503, ""), &state);
            let SpawnDecision::DeferClaimErrorByDrain { class, .. } = decision else {
                panic!("{decision:?}");
            };
            let expected = if matches!(state, CoordDrainState::Drained { .. }) {
                "device_drain:drained"
            } else {
                "device_drain:unknown"
            };
            assert_eq!(drain_stamp_reason(class), expected);
        }
    }
...
/// Plan `2026-09-13-drained-runner-never-reaches-idle`, review round 1: a coord
/// launch deferred by the drain is held and released, never reported failed,
/// and a replayed delivery cannot start a second copy.
#[cfg(test)]
mod launch_hold_tests {
    use super::*;
    use crate::agent_authorization::SpawnDecision as Authz;
    use crate::coord_drain_state::DeferClass;
    use std::sync::atomic::{AtomicUsize, Ordering};
    use std::sync::Arc;

    fn deferred() -> Authz {
        Authz::DeferredByDrain {
            reason: "coord has drained this device".into(),
    ...
    #[tokio::test]
    async fn a_deferred_launch_is_held_then_released_and_admitted() {
        let calls = Arc::new(AtomicUsize::new(0));
        let waits = Arc::new(AtomicUsize::new(0));
        let stop = tokio_util::sync::CancellationToken::new();
        let (c, w) = (calls.clone(), waits.clone());
        let out = hold_launch_for_drain(
            move || {
        ...
        .await;
        assert_eq!(out, LaunchHold::Admitted(Authz::Allow));
        assert_eq!(
            calls.load(Ordering::SeqCst),
            3,
            "re-authorized after each release"
        );
        assert_eq!(waits.load(Ordering::SeqCst), 2, "held once per deferral");
    }

    #[tokio::test]
    async fn a_held_launch_is_stopped_by_the_stop_token_without_admitting() {
        let stop = tokio_util::sync::CancellationToken::new();
        let stopper = stop.clone();
        let out = hold_launch_for_drain(
            || async { deferred() },
            move || {
                // The drain never lifts; coord's stop request arrives instead.
                stopper.cancel();
                std::future::pending::<()>()
            },
            &stop,
        ...
        .await;
        assert_eq!(out, LaunchHold::Stopped);
    }

    #[tokio::test]
    async fn a_registry_refusal_is_not_held() {
        let stop = tokio_util::sync::CancellationToken::new();
        let refusal = Authz::Deny {
        ...
        let out = hold_launch_for_drain(
            ...
            || async { panic!("a refusal must not wait for the drain") },
            &stop,
        )
        .await;
