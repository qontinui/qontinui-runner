...
//! so that wrapper went too, leaving exactly ONE injection primitive rather
//! than a delegating pair with a single remaining user.
...
/// Inject a message into a live session, given only an `AppHandle`.
///
/// `send_user_message` only ever needs the `AppHandle` (to reach the
/// `SessionManager` state), so callers that hold an `AppHandle` but no
/// `Arc<ApiState>` — e.g. the PR shepherd's device-local author-notify (plan
/// `2026-07-04-runner-pr-shepherd` Phase 4), which runs inside the PR watcher
/// with only `PrWatcherDeps` — inject through here without constructing an
/// `ApiState`.
///
