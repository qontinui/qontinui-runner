...
/// Undo the COORD-side side effects `create_ai_session` performs before the
/// task-run insert has been awaited, when that insert then fails.
///
...
/// emitter is `None` whenever the agent-logs gate is off or its id was
/// unresolvable.
fn teardown_coord_session_for_failed_task_run(
    registrar: &crate::claude_session::coord_register::AiCoordRegistrar,
    emitter: Option<&crate::claude_session::coord_register::AgentLogEmitter>,
    task_run_id: &str,
) {
    registrar.close_session(task_run_id);
...
#[cfg(test)]
mod failed_task_run_teardown_tests {
    use super::*;
    ...
    /// starts the agent_logs emitter BEFORE the task-run INSERT is awaited (the
    ...
    fn failed_task_run_insert_leaves_no_coord_session() {
        ...
        // Exactly what the command does before the `join!`.
        let coord_session_id = reg
            .register_session(&task_run_id, "New Chat", None)
            .expect("registration enabled by default");
        assert_eq!(reg.session_id_for(&task_run_id), Some(coord_session_id));
        assert_eq!(
            reg.task_run_id_for(&coord_session_id).as_deref(),
            Some(task_run_id.as_str())
        );

        // PG is unreachable → `create_task_run` errs → this arm runs.
        teardown_coord_session_for_failed_task_run(&reg, None, &task_run_id);

        assert_eq!(
            reg.session_id_for(&task_run_id),
            None,
            "a failed task-run insert must not leave a coord session registered against a \
             task_run_id that has no row"
        );
        assert_eq!(
            reg.task_run_id_for(&coord_session_id),
            None,
            "the reverse mapping must be evicted too — a dangling coord id resolves to nothing"
        );
    }

    /// The teardown is idempotent and safe on the paths where nothing was
    /// registered (kill switch off, unresolvable id) and on a repeat call.
    #[test]
    fn teardown_is_idempotent_and_safe_when_nothing_was_registered() {
        let _env = env_lock();
        std::env::remove_var("QONTINUI_SESSION_AUTOMATION_REGISTER");
        let (reg, _dir) = registrar();
        let task_run_id = uuid::Uuid::new_v4().to_string();
        ...
        reg.register_session(&task_run_id, "New Chat", None)
            .expect("registration enabled by default");
        teardown_coord_session_for_failed_task_run(&reg, None, &task_run_id);
        // Repeat close of an already-closed session.
        teardown_coord_session_for_failed_task_run(&reg, None, &task_run_id);
