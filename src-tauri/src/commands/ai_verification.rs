//! AI Verification Agent Tauri Commands
//!
//! This module provides Tauri commands for the AI verification agent that
//! enables AI-driven exploration and verification of application states.
...
use crate::verification_agent::{ExplorationStrategy, VerificationTask, VerificationTaskConfig};

/// Start an AI verification task
///
/// Begins verification of the application using the specified strategy.
/// The task will explore states, capture screenshots, and report discrepancies.
///
/// # Arguments
/// * `config` - Verification task configuration
...
#[tauri::command]
pub async fn start_verification_task(
    config: VerificationTaskConfig,
    state: State<'_, Arc<AppState>>,
) -> Result<CommandResponse, String> {
    info!(
        "Starting verification task for {} with strategy {}",
    ...
    let task = VerificationTask::new(config.clone(), state.inner().clone());
    ...
    match result {
        Ok(Ok(verification_result)) => {
            info!(
                ...
                verification_result.run_id, verification_result.summary
            );

            Ok(CommandResponse {
                success: true,
                message: Some("Verification task completed".to_string()),
                data: Some(serde_json::to_value(&verification_result).unwrap_or_default()),
            })
        }
        Ok(Err(e)) => {
...
#[tauri::command]
pub async fn get_verification_strategies() -> Result<CommandResponse, String> {
    let strategies = vec![
        serde_json::json!({
            "id": "exhaustive",
            "name": "Exhaustive",
...
///
/// Creates a verification plan showing which states and transitions would be
/// verified with the given configuration. Useful for previewing before running.
///
/// # Arguments
/// * `config` - Verification task configuration
/// * `state` - Application state
...
#[tauri::command]
pub async fn preview_verification_plan(
    config: VerificationTaskConfig,
    state: State<'_, Arc<AppState>>,
) -> Result<CommandResponse, String> {
    info!("Generating verification plan for {}", config.config_path);
    ...
    let graph = crate::verification_agent::StateMachineGraph::from_config(&config_value);

    let strategy = ExplorationStrategy::from_str(&config.strategy);
    let mut explorer = crate::verification_agent::StateExplorer::new(graph.clone(), strategy);
...
///
/// Returns a list of past verification runs.
///
/// # Arguments
/// * `limit` - Maximum number of runs to return (default: 20)
///
/// # Returns
/// * `Ok(CommandResponse)` - List of verification runs in data
#[tauri::command]
pub async fn get_verification_history(limit: Option<u32>) -> Result<CommandResponse, String> {
    let limit = limit.unwrap_or(20);

    // Read from verification reports directory
    ...
    if !reports_dir.exists() {
        return Ok(CommandResponse {
            success: true,
            message: Some("No verification history found".to_string()),
    ...
    Ok(CommandResponse {
        success: true,
        message: Some(format!("Found {} verification runs", runs.len())),
...
///
/// Returns the full verification report for a given run ID.
///
/// # Arguments
/// * `run_id` - The verification run ID
///
/// # Returns
/// * `Ok(CommandResponse)` - Full report in data
/// * `Err(String)` - Error if report not found
#[tauri::command]
pub async fn get_verification_report(run_id: String) -> Result<CommandResponse, String> {
    let reports_dir = std::path::PathBuf::from(
        r"C:\Users\Joshua\Documents\qontinui_parent_directory\.dev-logs\verification",
    );
...
/// Analyze a verification report with AI
///
/// Takes a verification report and returns an AI-ready prompt for analysis.
...
/// * `Err(String)` - Error if report not found
#[tauri::command]
pub async fn get_verification_analysis_prompt(run_id: String) -> Result<CommandResponse, String> {
    let reports_dir = std::path::PathBuf::from(
        r"C:\Users\Joshua\Documents\qontinui_parent_directory\.dev-logs\verification",
    );
    let report_path = reports_dir.join(format!("verification-report-{}.json", run_id));
...
///
/// Removes all verification reports older than the specified number of days.
///
/// # Arguments
...
pub async fn clear_verification_history(
    ...
    if !reports_dir.exists() {
        return Ok(CommandResponse {
            success: true,
            message: Some("No verification history to clear".to_string()),
    ...
    info!("Removed {} old verification reports", removed);

    Ok(CommandResponse {
        success: true,
        message: Some(format!("Removed {} verification reports", removed)),
