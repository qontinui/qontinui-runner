...
pub async fn run_flow_execution(
    ...
    // checkpoint_db removed — flow learning recording was SQLite-only, no-op
    let _ = (success, &flow, &state);
...
pub async fn pause_flow_execution(
    ...
    if let Some(state) = storage.executions.get_mut(&instance_id) {
        ...
        // Goes through `emit_flow_event` so the wire format is generated by
        // the `FlowEvent::FlowPaused` serde derive — matches the json!
        // literal previously used (key order ignored by JSON consumers,
        // `step_id` skipped when None instead of emitted as null).
        if let Some(flow_id) = flow_id {
            emit_flow_event(
                &app_handle,
                FlowEvent::FlowPaused {
                    instance_id: instance_id.clone(),
                    flow_id,
...
pub async fn resume_flow_execution(
    ...
    // Emit resume event via the typed `FlowEvent::FlowResumed` variant. Wire
    // format identical to the previous json! literal modulo `step_id`
    // serialization when None (now skipped instead of emitted as null).
    emit_flow_event(
        &app_handle,
