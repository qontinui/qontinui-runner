...
pub async fn ol_list_configs(
    ...
    storage.pg_db().list_ol_configs().await
}
...
pub async fn ol_get_config(
    ...
    storage.pg_db().get_ol_config(&id).await
}
...
pub async fn ol_save_config(
    ...
    let config = storage.pg_db().insert_ol_config(&request).await?;
    info!(
...
pub async fn ol_update_config(
    ...
    let config = storage.pg_db().update_ol_config(&id, &request).await?;
    info!(
...
pub async fn ol_delete_config(
    ...
    let deleted = storage.pg_db().delete_ol_config(&id).await?;
    if deleted {
