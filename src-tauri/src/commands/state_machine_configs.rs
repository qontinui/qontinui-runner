...
#[tauri::command]
pub async fn sm_list_configs(
    storage: State<'_, StorageCompartment>,
) -> Result<Vec<SmConfig>, String> {
    storage.pg_db().list_sm_configs().await
}
...
pub async fn sm_get_config(
    ...
    storage.pg_db().get_sm_config_full(&id).await
}
...
pub async fn sm_create_config(
    ...
    let config = storage.pg_db().insert_sm_config(&request).await?;
    info!(
...
pub async fn sm_update_config(
    ...
    let config = storage.pg_db().update_sm_config(&id, &request).await?;
    info!(
...
pub async fn sm_delete_config(
    ...
    let deleted = storage.pg_db().delete_sm_config(&id).await?;
    if deleted {
...
pub async fn sm_update_state(
    ...
    let state = storage.pg_db().update_sm_state(&id, &request).await?;
...
pub async fn sm_delete_state(
    ...
    storage.pg_db().delete_sm_state(&id).await
}
...
pub async fn sm_update_transition(
    ...
    let transition = storage.pg_db().update_sm_transition(&id, &request).await?;
    info!(
        "Updated transition: {} ({})",
        transition.name, transition.id
    );
...
pub async fn sm_delete_transition(
    ...
    storage.pg_db().delete_sm_transition(&id).await
}
...
pub async fn sm_import_config(
    ...
    let result = storage.pg_db().import_sm_config(&request).await?;
    info!(
...
pub async fn sm_get_thumbnails(
    ...
    storage.pg_db().get_sm_thumbnails(&config_id).await
}
...
pub async fn sm_get_capture_screenshots(
    ...
    storage.pg_db().get_capture_screenshots(&config_id).await
}
...
pub async fn sm_delete_capture_screenshots(
    ...
    storage.pg_db().delete_capture_screenshots(&config_id).await
}
...
pub async fn sm_audit_capture_screenshot_bounds(
    ...
    storage.pg_db().audit_capture_screenshot_bounds().await
}
