...
//!
//! Runs ELEVEN ordered checks: seven BLOCKING ones that stop at the first red
//! and report that link + its fix, plus four ADVISORY ones that always run
...
//! Green on every blocking check ⇒ "this runner can set gates." The output is
...
mod tests {
    ...
    #[test]
    fn check_specs_has_exactly_eleven_entries_seven_blocking_four_advisory() {
        ...
        // it, `coord_reachable`, is stronger and still blocking. `tenant_bindings`
        // is advisory for its own reason: a binding drift does not stop the
        // pinned slot's JWT from registering a gate.
        assert_eq!(CHECK_SPECS.iter().filter(|s| !s.advisory).count(), 7);
        assert_eq!(CHECK_SPECS.iter().filter(|s| s.advisory).count(), 4);
    }
        ...
        let checks: Vec<Check<'static>> = CHECK_SPECS
            ...
            .map(|spec| {
                ...
                Check::from_spec(spec, move || {
                    (
                        !red,
                        if red { "no .mcp.json here" } else { "green" }.to_string(),
                    )
                })
            })
            .collect();
    ...
    fn a_red_coord_reachable_still_blocks_even_though_it_always_runs() {
        ...
        assert!(
            !report.overall_ok,
            "a runner that cannot reach coord cannot set gates"
        );
        assert_eq!(
            report
                .checks
