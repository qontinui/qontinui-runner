...
macro_rules! row_to_workflow {
    ($r:expr) => {{
        UnifiedWorkflow {
            ...
            timeout_seconds: if $r.timeout_seconds == 0 { None } else { Some($r.timeout_seconds as u64) },
            ...
            workflow_architecture: {
                ...
                if s.is_empty() { None } else { serde_json::from_str(&format!("\"{}\"", s)).ok() }
            },
...
impl PgDb {
    ...
    pub async fn get_unified_workflow_by_name(&self, name: &str) -> Result<Option<UnifiedWorkflow>, String> {
        let conn = self.pool.get().await.map_err(|e| format!("PG pool error: {}", e))?;
    ...
    pub async fn create_unified_workflow_with_id(
        ...
        let setup_json = serde_json::to_string(&request.setup_steps).unwrap_or_else(|_| "[]".to_string());
        let verification_json = serde_json::to_string(&request.verification_steps).unwrap_or_else(|_| "[]".to_string());
        let agentic_json = serde_json::to_string(&request.agentic_steps).unwrap_or_else(|_| "[]".to_string());
        let completion_json = serde_json::to_string(&request.completion_steps).unwrap_or_else(|_| "[]".to_string());
        let log_sel_json = request.log_source_selection.as_ref()
            .map(|ls| serde_json::to_string(ls).unwrap_or_else(|_| "\"default\"".to_string()))
            .unwrap_or_else(|| "\"default\"".to_string());
        let ctx_json = request.context_ids.as_ref()
            .map(|ids| serde_json::to_string(ids).unwrap_or_else(|_| "[]".to_string()))
            .unwrap_or_else(|| "[]".to_string());
        let dis_ctx_json = request.disabled_context_ids.as_ref()
            .map(|ids| serde_json::to_string(ids).unwrap_or_else(|_| "[]".to_string()))
            .unwrap_or_else(|| "[]".to_string());
        let hc_urls_json = request.health_check_urls.as_ref()
            .map(|urls| serde_json::to_string(urls).unwrap_or_else(|_| "[]".to_string()))
            .unwrap_or_else(|| "[]".to_string());
        let stages_json = serde_json::to_string(&request.stages).unwrap_or_else(|_| "[]".to_string());
        let model_ov_json = serde_json::to_string(&request.model_overrides.clone().unwrap_or_default())
            .unwrap_or_else(|_| "{}".to_string());
        let constraint_ov_json = serde_json::to_string(&request.constraint_overrides.clone().unwrap_or_default())
            .unwrap_or_else(|_| "{}".to_string());
        let tool_tags_json = serde_json::to_string(&request.tool_tags).unwrap_or_else(|_| "[]".to_string());
        ...
        let rollback = request.rollback_policy.clone().unwrap_or_else(|| "none".to_string());
    ...
    pub async fn update_unified_workflow(
        ...
        let existing = self.get_unified_workflow(id).await?
        ...
        let context_ids = request.context_ids.as_ref().unwrap_or(&existing.context_ids);
        let disabled_context_ids = request.disabled_context_ids.as_ref().unwrap_or(&existing.disabled_context_ids);
        let auto_include_contexts = request.auto_include_contexts.unwrap_or(existing.auto_include_contexts);
        let log_watch_enabled = request.log_watch_enabled.unwrap_or(existing.log_watch_enabled);
        let health_check_enabled = request.health_check_enabled.unwrap_or(existing.health_check_enabled);
        let health_check_urls = request.health_check_urls.as_ref().unwrap_or(&existing.health_check_urls);
        let preflight_check_enabled = request.preflight_check_enabled.unwrap_or(existing.preflight_check_enabled);
        let enable_sweep = request.enable_sweep.unwrap_or(existing.enable_sweep);
        let max_sweep_iterations = request.max_sweep_iterations.unwrap_or(existing.max_sweep_iterations);
        ...
        let constraint_overrides = request.constraint_overrides.as_ref().unwrap_or(&existing.constraint_overrides);
        ...
        let completion_prompts_first = request.completion_prompts_first.unwrap_or(existing.completion_prompts_first);
        let model_overrides = request.model_overrides.as_ref().unwrap_or(&existing.model_overrides);
        let dependency_graph = request.dependency_graph.as_ref().or(existing.dependency_graph.as_ref());
        let cost_annotations = request.cost_annotations.as_ref().or(existing.cost_annotations.as_ref());
        let quality_report = request.quality_report.as_ref().or(existing.quality_report.as_ref());
        let acceptance_criteria = request.acceptance_criteria.as_ref().or(existing.acceptance_criteria.as_ref());
        ...
        let rollback_policy = request.rollback_policy.as_deref()
            .or(existing.rollback_policy.as_deref()).unwrap_or("none");
        let enforce_token_budget = request.enforce_token_budget.unwrap_or(existing.enforce_token_budget);
        let flow_control_json = request.flow_control_json.as_ref().or(existing.flow_control_json.as_ref());
        let phase_timeouts_json = request.phase_timeouts_json.as_ref().or(existing.phase_timeouts_json.as_ref());
        let workflow_architecture = request.workflow_architecture.as_ref()
        ...
        let verification_json = serde_json::to_string(verification_steps).unwrap_or_else(|_| "[]".to_string());
        let agentic_json = serde_json::to_string(agentic_steps).unwrap_or_else(|_| "[]".to_string());
        let completion_json = serde_json::to_string(completion_steps).unwrap_or_else(|_| "[]".to_string());
        let log_sel_json = serde_json::to_string(log_source_selection).unwrap_or_else(|_| "\"default\"".to_string());
        let ctx_json = serde_json::to_string(context_ids).unwrap_or_else(|_| "[]".to_string());
        let dis_ctx_json = serde_json::to_string(disabled_context_ids).unwrap_or_else(|_| "[]".to_string());
        let hc_urls_json = serde_json::to_string(health_check_urls).unwrap_or_else(|_| "[]".to_string());
        let stages_json = serde_json::to_string(stages).unwrap_or_else(|_| "[]".to_string());
        let model_ov_json = serde_json::to_string(model_overrides).unwrap_or_else(|_| "{}".to_string());
        let constraint_ov_json = serde_json::to_string(constraint_overrides).unwrap_or_else(|_| "{}".to_string());
    ...
    pub async fn search_unified_workflows(&self, query: &str) -> Result<Vec<UnifiedWorkflow>, String> {
        let conn = self.pool.get().await.map_err(|e| format!("PG pool error: {}", e))?;
    ...
    pub async fn list_slash_command_sources(&self) -> Result<Vec<(String, String, String)>, String> {
        let conn = self.pool.get().await.map_err(|e| format!("PG pool error: {}", e))?;
