...
fn parse_machine_json(bytes: &[u8]) -> LocalMachineIdentity {
    LocalMachineIdentity::from_machine_json(&crate::ambient::parse_machine_json(bytes))
}

impl LocalMachineIdentity {
    /// A parsed `machine.json` is "present" only when it carries an identity
    /// (`device_id` or the legacy `machine_id`); a file with neither — or an
    /// unreadable one, which the seam folds to the same all-`None` shape — is,
    /// for enrolment, the same as no file. The historical reader required the
...
pub fn local_machine_identity() -> LocalMachineIdentity {
    LocalMachineIdentity::from_machine_json(&crate::ambient::read_machine_json())
}
