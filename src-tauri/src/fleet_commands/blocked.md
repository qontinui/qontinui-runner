...
| A specific change landing in a file that **already exists** | `content_matches` | `{repo, path, pattern, on_ref?}` — a Rust regex over the decoded file contents (pattern ≤512 bytes, line-anchored `^`/`$`); a non-compiling or empty-matching pattern is refused at registration, a file not there yet stays `open`, a file over ~1 MB evaluates `misconfigured`. Prefer `file_exists` when the change creates the file (canonical: `_gate-registration`) |
