...
carries one; `continuation_dropped_born_cleared:` drops only the
continuation while leaving a healthy gate, on ANY predicate kind (it branches
on the verdict, not the predicate); and `deploy_order_predicate_weaker:` says
only that a deploy-shaped `pr_merged` gate clears EARLIER than the condition
its `phase_name` describes (the deploy-order row above). That last one fires on
the same predicate kind as the orchestrated-repo steer, so the two co-occur and
a healthy gate returns **at least two**
warnings. Branch on the PREFIX, never on the count: branching on emptiness
withdraws good gates on routine runs — which at the session-close door means
throwing away the one watcher the blocker had.
