...
history had not followed. Three repos (`qontinui-mcp`, `qontinui-hal-mcp`,
`qontinui-prm`) still declare a `commit-msg` pre-commit hook that rejects those
trailers when armed — a conflict surfaced as a POLICY_GAP, not resolved here.
