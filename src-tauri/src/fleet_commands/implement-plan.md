...
- **`fork_risk`, or a non-empty `forking_siblings`** — no hard holder, but sibling
  PRs are already racing this plan. This is **not** a clear: name the siblings in
  your first text turn and read them before proceeding, exactly as Step 0.7.5
  requires for a racing semantic resource.
...
`continuation_consumed_at` / `continuation_cancelled_at` fields. (`/coord/agent-gates`
is the **device-authed** read door — the operator `GET /coord/gates` is
`TenantId`-only and 403s this session's device JWT. The **cancel is device-authed too**
— `POST /coord/gates/<id>/agent/continuation-cancel`, the `/agent/` infix twin —
so this session does the whole loop itself. This parenthetical used to say "the
...
|---|---|---|
| `continuation_spawn != null` ∧ `dispatched_at == null` | **PRE-DISPATCH — the net is ARMED.** | **Mute or reject the GATE** — from an agent session, `POST .../coord/gates/<id>/agent/mute` or `.../agent/reject`; `run_gate_sweep` skips muted gates, and dispatch only happens on clear, so muting genuinely suppresses it. Or `coord_withdraw_gate` — LIVE. Do **NOT** POST `continuation-cancel` — there is nothing dispatched to cancel; it 404s. |
| `dispatched_at != null` ∧ `consumed_at == null` ∧ `cancelled_at == null` | Dispatched, not yet consumed. | `POST $COORD_HTTP_URL/coord/gates/<gate_id>/agent/continuation-cancel` `{reason}` — **the `/agent/` INFIX twin**; `cancelled_by` is derived from your JWT, not sent. The Step 0.6 call, with the response handling below. |
...
- **401 `operator context missing; SSO required`** — **you used the OPERATOR
  route.** This is not a permissions wall, it is a wrong door: retry the
  `/agent/` infix twin `POST .../coord/gates/<gate_id>/agent/continuation-cancel`
  with `{reason}` only. Reporting "the net is still armed, a redundant terminal
  may still spawn" on the strength of this 401 is exactly the mistake that let
  gate `7902e457` spawn a redundant session against a SHIPPED plan on
  2026-08-20. Only if the TWIN also fails is the net genuinely still armed.
...
**Second line of defence, when the cancel does not land.** On the agent twin the
cancel normally DOES land (`200 {"cancelled":true}`), so this is a genuine
fallback now rather than the usual outcome. A continuation that spawns anyway
...
2. Query the work unit's gates for a pending continuation:
   `GET $COORD_HTTP_URL/coord/agent-gates?work_unit_id=<id>` → rows where
   `continuation_dispatched_at != null ∧ continuation_consumed_at == null ∧
   continuation_cancelled_at == null`.
3. For each such gate, fire the **`/agent/` infix twin** (device or agent JWT;
   tenant and `cancelled_by` both derive server-side from the token, so the body
...
`{reason}` — the device-authed twin, so a device session does the WHOLE loop
itself: `/coord/agent-gates` discovers the pending continuation and
`/agent/continuation-cancel` retires it. (The unprefixed route is the operator's
and answers an agent 401.) Best-effort, never blocking:
