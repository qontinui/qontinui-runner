...
**The fixtures — run them, do not read them.** `bash <workspace-root>/qontinui-claude-config/scripts/steward-red-main-throttle-fixtures-test.sh`
...
`bash scripts/merge-payload-guard.sh` — a path relative to *this* repo, while P1 requires the <!-- bare-door-path-ok: this QUOTES the rejected bare spelling; rooting it would destroy the sentence (check #71) -->
...
**The fixtures — run them, do not read them.** `bash <workspace-root>/qontinui-claude-config/scripts/steward-empty-diff-fixtures-test.sh`
(this repo; its tracked `.sh` files are mode `100644`, so invoke through `bash`, never as a
...
curl -sS -m 90 -H @"$hdr" "https://coord.qontinui.io/pr-merge/prs" \
  | bash <workspace-root>/qontinui-claude-config/scripts/conflict-triage.sh --it <N> \
...
**The fixtures — run them, do not read them.** `bash <workspace-root>/qontinui-claude-config/scripts/steward-conflict-triage-fixtures-test.sh`
