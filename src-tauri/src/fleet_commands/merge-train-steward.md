...
| C38 an all-deletions PR, landed, whose N has decayed (`main` later re-adds the file) | `NOT-NOOP merge conflicts` | pass | 1 | the **Mn guard** pin. Mn reads `NOOP-PROVEN`, but B at the stamp prints `paths=1 but no compared blob exists in the base`, which is M's refusal (c), so the PR must NOT close. Fails if the guard is dropped |
