...
are owned by `/vet-plan` and `/implement-plan`. An `IN PROGRESS` stamp
specifically is CONDITIONALLY overwritable here too, on the same
principle `/vet-plan` applies: only overwrite it when verification has
proved the stamp wrong, never as a routine re-run. If `/verify-plan-status`
