...
An `IN PROGRESS` stamp is a third case worth naming explicitly: it
specifically is CONDITIONALLY overwritable by `/vet-plan` under that
skill's own rule (route to closeout instead of re-vetting when the work
already shipped). This orchestrator does not restate that rule — it only
relays whatever disposition `/vet-plan` reaches.
