...
# --- T13: a 200 from the ownership door about ANOTHER repo is not a proof (the
...
run_coord "coord:stacked-on=#6"
expect_rc 5 "T13 door 200 about another repo"
expect_calls post 0 "T13"

# --- T14: the vendored library is byte-identical to scripts/lib's. It ships in
...
if [[ ! -r "$_lib_here" ]]; then fail "T14: $_lib_here is missing (the bundled skill would have no owner-tenant proof)"
...
else fail "T14: lib/coord-tenant-credential.sh differs from scripts/lib/coord-tenant-credential.sh -- copy scripts/lib's forward"; fi
