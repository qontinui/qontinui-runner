...
/// Test-only env isolation, folded into the lib's [`ambient::test_support`]
/// (plan `2026-09-03-runner-tests-read-ambient-machine-state`): ONE
/// process-wide `env_lock()`, `EnvVarRestore`, and the `isolated_ambient()`
/// fixture. Kept under this name so every `crate::test_env::…` path resolves.
#[cfg(test)]
pub(crate) mod test_env {
    pub(crate) use crate::ambient::test_support::*;
}

/// Arm the ambient canary before any test in this binary runs (plan D3): a
/// seam read with no `isolated_ambient()` guard live then panics naming the
/// source, instead of silently measuring the developer's machine.
#[cfg(test)]
#[ctor::ctor]
fn arm_ambient_canary() {
    crate::ambient::test_support::arm();
}
