...
> {
    let result = if let Some(pg) = &state.app_state.pg_db {
        pg.list_unified_workflows().await
    } else {
        state.app_state.checkpoint_db.list_unified_workflows()
    };
    match result {
...
> {
    ...
    let local_result = if let Some(pg) = &state.app_state.pg_db {
    ...
    match client.fetch_workflow(&id).await {
        Ok(workflow) => {
            ...
            if let Some(pg) = &state.app_state.pg_db {
                if let Err(e) = pg.create_unified_workflow_with_id(&workflow.id, &create_req).await {
                    warn!("Failed to cache workflow locally (PG): {}", e);
                }
...
> {
    ...
    let result = if let Some(pg) = &state.app_state.pg_db {
        pg.create_unified_workflow(&request).await
    } else {
        state.app_state.checkpoint_db.create_unified_workflow(&request)
    };
    match result {
        Ok(created) => {
            info!(
...
> {
    ...
    let existing_for_feedback = if let Some(pg) = &state.app_state.pg_db {
        pg.get_unified_workflow(&id).await.ok().flatten()
    } else {
        state.app_state.checkpoint_db.get_unified_workflow(&id).ok().flatten()
    };
    ...
    let update_result = if let Some(pg) = &state.app_state.pg_db {
        pg.update_unified_workflow(&id, &request).await
    } else {
        state.app_state.checkpoint_db.update_unified_workflow(&id, &request)
    };
    match update_result {
        Ok(updated) => {
            info!(
...
pub async fn delete_unified_workflow(
    ...
    // Check if this workflow was generated by a task run (for feedback tracking)
    let existing_for_del = if let Some(pg) = &state.app_state.pg_db {
        pg.get_unified_workflow(&id).await.ok().flatten()
    } else {
        state.app_state.checkpoint_db.get_unified_workflow(&id).ok().flatten()
    };
    ...
    let del_result = if let Some(pg) = &state.app_state.pg_db {
        pg.delete_unified_workflow(&id).await
    } else {
        state.app_state.checkpoint_db.delete_unified_workflow(&id)
    };
    match del_result {
        Ok(true) => {
...
pub async fn search_unified_workflows(
    ...
    (StatusCode, Json<ApiResponse<()>>),
> {
    // PG search uses full-text; only for simple text queries without category/tag filters
    let search_result = if let Some(pg) = &state.app_state.pg_db {
        if query.category.is_none() && query.tag.is_none() {
            let q = query.q.as_deref().unwrap_or("");
            pg.search_unified_workflows(q).await
        } else {
            state.app_state.checkpoint_db.search_unified_workflows(&query)
        }
    } else {
        state.app_state.checkpoint_db.search_unified_workflows(&query)
    };
...
pub async fn run_unified_workflow(
    ...
    if has_prompt_steps {
        ...
        let input = CreateTaskRunInput::new(&execution_id, &workflow.name)
            ...
            .with_auto_continue(true)
            .with_workflow_type("unified");
        let create_err = if let Some(pg) = &state.app_state.pg_db {
...
async fn toggle_favorite_handler(
    ...
    let new_state = if let Some(pg) = &state.app_state.pg_db {
        pg.toggle_favorite(&id).await
    } else {
        state.app_state.checkpoint_db.toggle_unified_workflow_favorite(&id)
    }
    .map_err(|e| {
