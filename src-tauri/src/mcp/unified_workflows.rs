...
> {
    ...
    match client.fetch_workflow(&id).await {
        Ok(workflow) => {
            ...
            if let Err(e) = state.app_state.pg_db.create_unified_workflow_with_id(&workflow.id, &create_req).await {
...
> {
    ...
    match state.app_state.pg_db.create_unified_workflow(&request).await {
        Ok(created) => {
            info!(
...
> {
    ...
    let existing_for_feedback = state.app_state.pg_db.get_unified_workflow(&id).await.ok().flatten();
    ...
    match state.app_state.pg_db.update_unified_workflow(&id, &request).await {
        Ok(updated) => {
            info!(
...
pub async fn delete_unified_workflow(
    ...
    // Check if this workflow was generated by a task run (for feedback tracking)
    let existing_for_del = state.app_state.pg_db.get_unified_workflow(&id).await.ok().flatten();
...
pub async fn get_last_inline_workflow(
    ...
    match state.app_state.pg_db.get_setting("last_inline_workflow").await {
        Ok(Some(workflow)) => {
...
> {
    ...
    if let Ok(request_json) = serde_json::to_value(&request) {
        ...
        if let Err(e) = state.app_state.pg_db.set_setting("last_inline_workflow", &request_json).await {
...
pub async fn get_unified_workflow_stats(
    ...
    match state.app_state.pg_db.get_workflow_stats(&workflow.name).await {
...
async fn toggle_favorite_handler(
    ...
    let new_state = state.app_state.pg_db.toggle_favorite(&id).await
    .map_err(|e| {
