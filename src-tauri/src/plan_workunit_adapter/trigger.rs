...
struct LoopState {
    ...
    /// Whether the cold-start bulk seed has run for the current plans dir.
    /// Reset whenever `last_applied` is cleared for a new corpus, so a moved
    /// active dir is primed the same way a fresh process is.
    bulk_seeded: bool,
}
