...
fn cmd_env_capture(dry_run: bool) -> u8 {
    // Publish a lazy PG pool on this machine's bundled cluster so the
    // `db_schema` collector can run. Best-effort: a failure (typically "no
    // runner up, so the cluster is not running") just omits that section.
    if let Err(e) = crate::env_agent::publish_pg_pool_from_local_cluster() {
