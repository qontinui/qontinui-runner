...
//!
//! This module handles generating detailed reports of verification results
...
use super::types::{StateVerification, TransitionVerification, VerificationResult};
...
/// Report for a single state's verification
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl From<&StateVerification> for StateVerificationReport {
    fn from(v: &StateVerification) -> Self {
        let passed = matches!(v.status, super::types::VerificationStatus::Passed);
...
/// Report for a transition's verification
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TransitionVerificationReport {
    /// Transition ID
...
impl From<&TransitionVerification> for TransitionVerificationReport {
    fn from(v: &TransitionVerification) -> Self {
        let passed = matches!(v.status, super::types::VerificationStatus::Passed);
...
pub struct VerificationReport {
    ...
    pub state_reports: Vec<StateVerificationReport>,

    /// Detailed transition reports
    pub transition_reports: Vec<TransitionVerificationReport>,
...
impl VerificationReport {
    /// Create a report from verification result
    pub fn from_result(result: &VerificationResult, config_name: String) -> Self {
        let state_reports: Vec<StateVerificationReport> = result
            .state_verifications
            .iter()
            .map(StateVerificationReport::from)
            .collect();

        let transition_reports: Vec<TransitionVerificationReport> = result
            .transition_verifications
            .iter()
            .map(TransitionVerificationReport::from)
            .collect();
        ...
        let summary = ReportSummary {
            ...
            states_verified: result.states_visited,
            ...
            transitions_verified: result.transitions_verified,
    ...
    /// Build a prompt for AI analysis of the verification results
    fn build_ai_analysis_prompt(summary: &ReportSummary, discrepancies: &[Discrepancy]) -> String {
        let mut prompt = String::new();

        prompt.push_str("# Verification Results Analysis\n\n");
        prompt.push_str(
            "Please analyze the following verification results and provide recommendations.\n\n",
        );

        prompt.push_str("## Summary\n");
        prompt.push_str(&format!(
            "- States verified: {}/{} ({:.1}% pass rate)\n",
            summary.states_passed,
            summary.states_verified,
            summary.overall_pass_rate * 100.0
        ));
        prompt.push_str(&format!(
            "- Transitions verified: {}/{} ({} passed, {} failed)\n",
        ...
        if !discrepancies.is_empty() {
            ...
            prompt.push_str("All verification checks passed. No discrepancies found.\n");
        }
    ...
    pub fn save_json(&self, output_dir: &PathBuf) -> Result<PathBuf, String> {
        ...
        let filename = format!("verification-report-{}.json", self.run_id);
    ...
    pub fn save_markdown(&self, output_dir: &PathBuf) -> Result<PathBuf, String> {
        ...
        let filename = format!("verification-report-{}.md", self.run_id);
        let path = output_dir.join(&filename);
        ...
        // State details
        md.push_str("## State Verification Details\n\n");
