...
//!
//! This module implements the main verification task that orchestrates
//! state machine exploration and verification.
...
use super::types::{
    ElementCheck, StateVerification, TransitionVerification, VerificationResult,
    VerificationStatus, VerificationTaskConfig,
};

use crate::commands::AppState;

/// Events emitted during verification for real-time updates
...
/// Main verification task that executes the verification run
pub struct VerificationTask {
    config: VerificationTaskConfig,
    app_state: Arc<AppState>,
    result: VerificationResult,
    event_sender: Option<mpsc::Sender<VerificationEvent>>,
    start_time: Instant,
}

impl VerificationTask {
    /// Create a new verification task
    pub fn new(config: VerificationTaskConfig, app_state: Arc<AppState>) -> Self {
        let run_id = uuid::Uuid::new_v4().to_string();
        let result =
            VerificationResult::new(run_id, config.config_path.clone(), config.strategy.clone());
    ...
    pub fn with_event_sender(mut self, sender: mpsc::Sender<VerificationEvent>) -> Self {
        self.event_sender = Some(sender);
        self
    }

    /// Execute the verification task
    pub async fn execute(mut self) -> Result<VerificationResult, String> {
        info!(
            "Starting verification task {} for {}",
        ...
        // Send started event
        self.send_event(VerificationEvent::Started {
        ...
        for (index, state_visit) in path.states.iter().enumerate() {
            ...
            {
                warn!("Verification timeout reached");
                break;
            }
            ...
            self.send_event(VerificationEvent::StateStarted {
            ...
            let verification = self
                .verify_state(&state_visit.state_id, &state_visit.state_name, state_info)
                .await;

            // Check if we should stop on failure
            if self.config.stop_on_first_failure
                && matches!(verification.status, VerificationStatus::Failed)
            {
                self.result.add_state_verification(verification.clone());

                self.send_event(VerificationEvent::StateCompleted {
                    state_id: verification.state_id.clone(),
                    state_name: verification.state_name.clone(),
                    status: format!("{:?}", verification.status),
                    duration_ms: verification.duration_ms,
                    discrepancy_count: verification
                ...
                warn!(
                    "Stopping verification due to failure at state {}",
                    verification.state_id
                );
                break;
            }

            let discrepancy_count = verification
                .expected_elements_found
                .iter()
                .filter(|e| !e.found)
                .count() as u32
            ...
            self.result.add_state_verification(verification);
        ...
        // Verify transitions if configured
        for transition_step in &path.transitions {
            if !transition_step.verify {
                continue;
            }

            self.send_event(VerificationEvent::TransitionStarted {
            ...
            self.send_event(VerificationEvent::TransitionCompleted {
                transition_id: verification.transition_id.clone(),
                status: format!("{:?}", verification.status),
                duration_ms: verification.duration_ms,
            })
            .await;

            self.result.add_transition_verification(verification);
        }
        ...
        let report = VerificationReport::from_result(&self.result, config_name);
        ...
        match report.save_json(&PathBuf::from(&output_dir)) {
            Ok(path) => {
                ...
                info!("Verification report saved to {:?}", path);
            ...
            Err(e) => {
                error!("Failed to save verification report: {}", e);
            }
        }

        // Send completion event
        self.send_event(VerificationEvent::Completed {
            run_id: self.result.run_id.clone(),
    ...
    /// Verify a single state using Python bridge for actual state detection
    async fn verify_state(
        ...
        debug!("Verifying state: {} ({})", state_name, state_id);
        ...
        if let Some(ref path) = screenshot_path {
            self.send_event(VerificationEvent::ScreenshotCaptured {
                path: path.clone(),
    ...
    /// Verify a transition between states
    async fn verify_transition(
        ...
        let (status, error) = match transition_result {
            Ok(_) => (VerificationStatus::Passed, None),
            Err(e) => (VerificationStatus::Failed, Some(e)),
        };
    ...
    async fn capture_screenshot(&self, context: &str) -> Option<String> {
        let output_dir = self.config.output_directory.clone().unwrap_or_else(|| {
            r"C:\Users\Joshua\Documents\qontinui_parent_directory\.dev-logs\verification\screenshots".to_string()
        });
    ...
    async fn send_event(&self, event: VerificationEvent) {
        if let Some(ref sender) = self.event_sender {
            if let Err(e) = sender.send(event).await {
                debug!("Failed to send verification event: {}", e);
            }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_verification_result_complete() {
        ...
        assert!(matches!(result.overall_status, VerificationStatus::Failed));
