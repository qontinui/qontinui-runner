...
/// 17 -> 6, leaving 23 + 10 + 6 + 2 = 41. The `work-owed` sites that remain post to **qontinui-web**, which
...
fn every_defaulting_call_site_declares_its_tenant_scope() {
    ...
    assert_eq!(
        ...
        "expected 40 defaulting call sites — the Phase-2 census's 52 at ebbd3c70 minus the 12 \
         ...
         request carries no tenant field for anyone to set or derive); 50 to 51. Phase 6 then moved the 11 \
         ...
         canonical-repo fetch): 51 - 11 + 1 = 41. Phase 7 then moved fleet.rs's \
         tenant-declaring wire body onto the stating seam: 40. Found {sites}. A change here \
         is fine — it just has \
