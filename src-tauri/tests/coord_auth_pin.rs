...
fn every_defaulting_call_site_declares_its_tenant_scope() {
    ...
    assert_eq!(
        sites, 38,
        "expected 38 defaulting call sites — the Phase-2 census's 52 at ebbd3c70 minus the 12 \
         ...
         canonical-repo fetch): 49 - 11 + 1 = 39. Phase 7 then moved fleet.rs's \
         tenant-declaring wire body onto the stating seam: 38. Found {sites}. A change here \
         is fine — it just has \
