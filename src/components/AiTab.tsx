...
export function AiTab({
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      try {
        const response = await tracedFetch(`${getApiBase()}/workflow/resumable`, {
          signal: controller.signal,
        });
        const result = await response.json();
        if (!cancelled && result.success) {
  ...
  const handleResumeWorkflow = useCallback(async () => {
    ...
    setIsResuming(true);
    try {
      const response = await tracedFetch(`${getApiBase()}/workflow/resume`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      });
      const result = await response.json();
  ...
  const handleForceContinue = useCallback(async () => {
    ...
    try {
      ...
      const response = await tracedFetch(`${getApiBase()}/workflow/force-continue`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        ...
        className={`shrink-0 px-4 py-3 border-b ${
          statusInfo.status === "running"
            ...
            : statusInfo.status === "resumable"
              ? `${getAccentColors("orange").bg} ${getAccentColors("orange").border}`
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            {statusInfo.status === "running" ? (
              <div
                className={`w-9 h-9 rounded-full ${getStatusColors("running").bg} flex items-center justify-center`}
              >
                <Loader2 className={`w-4 h-4 ${getStatusColors("running").icon} animate-spin`} />
              </div>
            ) : statusInfo.status === "resumable" ? (
              <div
                className={`w-9 h-9 rounded-full ${getAccentColors("orange").bg} flex items-center justify-center`}
              >
                <Clock className={`w-4 h-4 ${getAccentColors("orange").text}`} />
              </div>
            ) : (
            ...
            <div>
              <div className="flex items-center gap-1.5">
                ...
                <PageTutorialMenu page="ai" variant="compact" />
              </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-2 py-1 rounded text-xs transition-colors ${
                autoFixEnabled
                  ? `${getAccentColors("blue").bg} ${getAccentColors("blue").text} hover:opacity-80`
                  : "bg-muted text-muted-foreground hover:bg-muted/80"
            ...
            {resumableTask && !isRunning && (
              <button
                ...
                disabled={isResuming}
                className={`flex items-center gap-2 px-3 py-1.5 ${getAccentColors("orange").bgSolid} text-white rounded-lg font-medium text-sm hover:opacity-90 disabled:opacity-50 transition-colors`}
              >
                {isResuming ? (
                  <>
            ...
            {!isRunning && !resumableTask && runFilteredAiOutputLines.length > 0 && (
              <button
                ...
                className={`flex items-center gap-2 px-3 py-1.5 ${getAccentColors("amber").bgSolid} text-white rounded-lg font-medium text-sm hover:opacity-90 disabled:opacity-50 transition-colors`}
            ...
            {isRunning && (
              <button
                ...
                disabled={isStopping}
                className={`flex items-center gap-2 px-3 py-1.5 ${getStatusColors("error").bg} ${getStatusColors("error").text} rounded-lg font-medium text-sm hover:opacity-90 disabled:opacity-50 transition-colors border ${getStatusColors("error").border}`}
              >
                {isStopping ? (
                  <>
                    <Loader2 className="w-3.5 h-3.5 animate-spin" />
        ...
        {lastResult && (
          <div
            className={`mt-2 flex items-center gap-2 text-xs ${
              lastResult.success ? getStatusColors("success").text : getStatusColors("error").text
            }`}
          >
            {lastResult.success ? (
        ...
        <div className="flex items-center gap-4">
                  ...
                  <div className="px-3 py-2 border-b border-border bg-muted/20 space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className={`flex items-center gap-1 px-2 py-1 text-xs ${getStatusColors("error").bg} ${getStatusColors("error").text} hover:opacity-80 rounded transition-colors disabled:opacity-50`}
                      >
                        {isDeleting ? (
                          <Loader2 className="w-3 h-3 animate-spin" />
                        ) : (
          ...
          <div className="flex items-center gap-4 text-xs">
            ...
            {findingsSummary.total > 0 ? (
              <div className="flex items-center gap-1.5">
                <AlertTriangle
                  className={`w-3.5 h-3.5 ${findingsSummary.critical > 0 ? getStatusColors("error").icon : getAccentColors("orange").text}`}
                />
                ...
                {findingsSummary.resolved > 0 && (
                  <span className={getStatusColors("success").text}>
                    ({findingsSummary.resolved} fixed)
                  </span>
                )}
              </div>
            ) : (
              <div className="flex items-center gap-1.5">
                <CheckCircle className={`w-3.5 h-3.5 ${getStatusColors("success").icon}`} />
          ...
          {selectedSessionId && selectedSessionId !== ALL_SESSIONS && runAutoContinue !== null && (
            <div className="ml-auto flex items-center gap-2">
              <button
                ...
                className={`flex items-center gap-1.5 px-2 py-1 rounded text-xs transition-colors ${
                  runAutoContinue
                    ? `${getAccentColors("purple").bg} ${getAccentColors("purple").text} hover:opacity-80`
                    : "bg-muted text-muted-foreground hover:bg-muted/80"
