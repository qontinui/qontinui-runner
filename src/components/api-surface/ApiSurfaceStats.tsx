      ...
      {diff && (
        <div className="text-xs bg-zinc-800/50 rounded px-3 py-2 space-y-1">
          ...
          {diff.addedCommands.length > 0 && (
            <DiffLine color="#22c55e" label={`+${diff.addedCommands.length} commands`} items={diff.addedCommands.map(c => c.name)} />
          )}
          {diff.removedCommands.length > 0 && (
            <DiffLine color="#ef4444" label={`-${diff.removedCommands.length} commands`} items={diff.removedCommands} />
          )}
          {diff.addedRoutes.length > 0 && (
            <DiffLine color="#22c55e" label={`+${diff.addedRoutes.length} routes`} items={diff.addedRoutes.map(r => `${r.method} ${r.path}`)} />
          )}
          {diff.removedRoutes.length > 0 && (
            <DiffLine color="#ef4444" label={`-${diff.removedRoutes.length} routes`} items={diff.removedRoutes} />
          )}
          {diff.addedPgMethods.length > 0 && (
            <DiffLine color="#22c55e" label={`+${diff.addedPgMethods.length} PgDb methods`} items={diff.addedPgMethods.map(m => m.name)} />
          )}
          {diff.removedPgMethods.length > 0 && (
            <DiffLine color="#ef4444" label={`-${diff.removedPgMethods.length} PgDb methods`} items={diff.removedPgMethods} />
          )}
          {diff.newOrphans.length > 0 && (
            <DiffLine color="#f97316" label={`+${diff.newOrphans.length} orphans`} items={diff.newOrphans.map(o => `${o.endpointType}: ${o.name}`)} />
          )}
          {diff.resolvedOrphans.length > 0 && (
            <DiffLine color="#22c55e" label={`-${diff.resolvedOrphans.length} orphans resolved`} items={diff.resolvedOrphans} />
          )}
          {diff.newConnections.length > 0 && (
            <DiffLine color="#3b82f6" label={`+${diff.newConnections.length} connections`} items={diff.newConnections.map(c => `${c.fromName} → ${c.toName}`)} />
          )}
          {diff.brokenConnections.length > 0 && (
            <DiffLine color="#ef4444" label={`-${diff.brokenConnections.length} connections broken`} items={diff.brokenConnections.map(c => `${c.fromName} → ${c.toName}`)} />
          )}
        </div>
      )}
    </div>
...
function DiffLine({ color, label, items }: { color: string; label: string; items: string[] }) {
  return (
    <div className="flex items-start gap-2">
      <span className="font-medium whitespace-nowrap" style={{ color }}>{label}</span>
      <span className="text-zinc-500 truncate">{items.slice(0, 5).join(", ")}{items.length > 5 ? ` (+${items.length - 5} more)` : ""}</span>
    </div>
  );
}
