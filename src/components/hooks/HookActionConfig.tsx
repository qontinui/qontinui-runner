...
export function HookActionConfig({ actionType, actionConfig, onChange }: HookActionConfigProps) {
  ...
  // Render command action config
  const renderCommandConfig = (config: CommandAction) => {
  ...
  // Render webhook action config
  const renderWebhookConfig = (config: WebhookAction) => {
  ...
  // Render log action config
  const renderLogConfig = (config: LogAction) => {
    return (
      <div className="space-y-4">
        <div>
  ...
  // Render notification action config
  const renderNotificationConfig = (config: NotificationAction) => {
    return (
      <div className="space-y-4">
        <div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="pt-4 border-t border-border">
        {actionType === "command" && renderCommandConfig(actionConfig as CommandAction)}
        {actionType === "webhook" && renderWebhookConfig(actionConfig as WebhookAction)}
        {actionType === "log" && renderLogConfig(actionConfig as LogAction)}
        {actionType === "notification" &&
          renderNotificationConfig(actionConfig as NotificationAction)}
      </div>
    </div>
  );
}
