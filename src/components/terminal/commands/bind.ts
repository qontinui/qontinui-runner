 ...
 * them now reaches this through `lib/ui-bridge/guardedHandler.ts` (applied
 * at the HANDLER, so the registration stays a plain literal carrying its
 * `effect`); the three
 ...
 * typing an untranslatable key's own name into a live PTY. A guarded handler
 * declared `valuesCheckedBy: "handler"` names EVERY declared field here,
 * because each already has a typed validator (`requireTextPayload`,
 * `requireMaxLines`) — the bag and key-set refusals still apply to it.
 */
