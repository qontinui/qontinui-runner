 ...
 *
 * Ported from qontinui-runner#1301. The guard now sits on each `handler`, so
 * the actions stay plain literals that carry main's `effect` annotations, and
 * `create-ai-session` answers through main's `buildAiSessionSpawnEnvelope`
 * (e910610f2) rather than the unconditional `success: true` it had then.
 */
...
describe("terminal launch menu", () => {
  ...
  it("keeps main's effect classification on every action", () => {
    // Moving the actions into a factory must not cost the `effect` calculus a
    // single annotation: `action-effect-coverage.test.ts` follows the factory
    // statically, and this pins the values at runtime.
    const { byId } = harness();
    expect(Object.fromEntries(IDS.map((id) => [id, byId[id].effect]))).toEqual({
      "create-plain": "write",
      "create-ai-session": "destructive",
      "create-best-account": "destructive",
      "create-with-command": "destructive",
    });
  });

  it("create-ai-session: a spawn that produced nothing is still a failure (e910610f2)", async () => {
    // The guard decides whether the bag may reach the launcher; the envelope
    // decides what the launcher's answer means. Both must hold at once.
    const launchAiSession = vi.fn(async () => [] as string[]);
    const actions = buildTerminalLaunchMenuActions({
      callRegistry: vi.fn() as never,
      launchAiSession,
    });
    const ai = actions.find((a) => a.id === "create-ai-session")!;
    await expect(ai.handler({ configDir: "/c", count: 2 })).rejects.toThrow();
    expect(launchAiSession).toHaveBeenCalledWith(2, "/c", undefined);
  });
