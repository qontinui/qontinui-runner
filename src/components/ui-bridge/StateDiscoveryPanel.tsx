...
type ViewMode = "states" | "transitions" | "matrix" | "stats";
...
function StateCandidateCard({
  ...
  const analysis = useMemo(() => {
    ...
    const dominantLandmark = Object.entries(landmarks).sort((a, b) => b[1] - a[1])[0]?.[0] || "unknown";
    ...
    <div className="border border-border/50 rounded-lg overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <Badge className={stateTypeColors[analysis.stateType]}>
            {analysis.stateType}
          </Badge>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <Badge
            variant={candidate.cooccurrenceRate === 1 ? "success" : "warning"}
            size="sm"
          >
          ...
          <div className="space-y-1">
            ...
            <div className="flex flex-wrap gap-1">
              {Object.entries(analysis.zones).map(([zone, count]) => (
                <Badge
                  key={zone}
                  className={`${ZONE_COLORS[zone] || "bg-muted"} border`}
                  size="sm"
                >
          ...
          {targetFp && (
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-2 p-2 bg-muted/30 rounded text-xs">
                ...
                <Badge variant="muted" size="sm">{targetFp.role}</Badge>
              ...
              <div className="max-h-32 overflow-y-auto space-y-1">
                {transition.appearedFingerprints.slice(0, 10).map((hash) => {
                  ...
                  return (
                    <div
                      key={hash}
                      className="flex items-center gap-2 p-1.5 bg-green-500/10 rounded text-xs"
                    >
                      <span className="font-mono text-[10px] text-muted-foreground">{hash}</span>
                      {fp && (
                        <>
                          <Badge variant="muted" size="sm">{fp.role || fp.tagName}</Badge>
                          {fp.accessibleName && (
              ...
              <div className="max-h-32 overflow-y-auto space-y-1">
                {transition.disappearedFingerprints.slice(0, 10).map((hash) => {
                  ...
                  return (
                    <div
                      key={hash}
                      className="flex items-center gap-2 p-1.5 bg-red-500/10 rounded text-xs"
                    >
                      <span className="font-mono text-[10px] text-muted-foreground">{hash}</span>
                      {fp && (
                        <>
...
function StatsView({ data }: { data: CooccurrenceExport }) {
  const stats = useMemo(() => {
    ...
    Object.values(data.fingerprintDetails).forEach((fp) => {
      ...
      landmarkDistribution[fp.landmarkContext] = (landmarkDistribution[fp.landmarkContext] || 0) + 1;
    ...
    return {
      ...
      repeatingCount,
      avgAppearances: data.allFingerprints.length > 0
            ...
            .map(([size, count]) => {
              ...
              return (
                <div key={size} className="flex items-center gap-2">
                  <div className="w-24 text-xs text-muted-foreground">{SIZE_LABELS[size] || size}</div>
                  <div className="flex-1 h-4 bg-muted/30 rounded overflow-hidden">
                    <div
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2 text-yellow-500">
          ...
          <span className="text-sm">
            Matrix too large to display ({matrixSize}x{matrixSize}).
...
export function StateDiscoveryPanel({
  ...
  const handleExportData = () => {
    if (!cooccurrenceData) return;
    const blob = new Blob([JSON.stringify(cooccurrenceData, null, 2)], { type: "application/json" });
      ...
      <div className="flex flex-col items-center justify-center h-full text-muted-foreground text-sm gap-4">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs max-w-md">
            Start a capture session from the toolbar to begin collecting element
            co-occurrence data. Visit multiple pages and perform actions to build
            a state model.
          </p>
        </div>
      </div>
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-muted-foreground text-sm gap-4">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs max-w-md">
            Refresh elements or navigate to different pages to capture element data.
            Perform actions to record state transitions.
          </p>
        </div>
      </div>
    );
        ...
        <div className="flex items-center gap-2">
          ...
          {onRunDiscovery && (
            <Button
              ...
              disabled={isRunningDiscovery || !cooccurrenceData || cooccurrenceData.presenceMatrix.length < 2}
              title={
                !cooccurrenceData
                  ? "No data to analyze"
            ...
            >
              <Fingerprint className={`w-4 h-4 mr-1.5 ${isRunningDiscovery ? "animate-pulse" : ""}`} />
          ...
          <div className="space-y-2">
            {cooccurrenceData.transitions.length === 0 ? (
              <div className="text-center text-muted-foreground text-sm py-8">
                ...
                <p className="text-xs mt-1">
                  Execute actions to record state transitions
                </p>
              </div>
            ) : (
