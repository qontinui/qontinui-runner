 ...
 *
 * Displays history of past verification runs with summary information.
 */
...
import type { VerificationHistoryItem } from "../../types/verification-agent";
...
export function VerificationHistory({ onSelectRun }: VerificationHistoryProps) {
  const { history, historyLoading, refreshHistory, clearHistory, error } = useVerificationAgent();

  const handleClearHistory = useCallback(async () => {
    if (window.confirm("Clear verification reports older than 30 days?")) {
      const removed = await clearHistory(30);
      if (removed > 0) {
        console.log(`Removed ${removed} old verification reports`);
      }
    }
  }, [clearHistory]);
  ...
  const getStatusIcon = (item: VerificationHistoryItem) => {
  ...
  const getStatusBadge = (item: VerificationHistoryItem) => {
    const status = getStatusFromSummary(item.summary);
    switch (status) {
      case "passed":
        return (
      ...
      <div className="flex-1 overflow-auto p-4">
        {historyLoading && history.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-muted-foreground">
            ...
            <p className="text-lg font-medium">No Verification Runs</p>
            <p className="text-sm mt-2">Run a verification task to see history here</p>
          </div>
        ) : (
