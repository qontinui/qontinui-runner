 ...
 *
 * Main container for AI Verification Agent.
 * Allows configuration and execution of verification tasks.
 */
...
import type { VerificationTaskConfig, VerificationPlan } from "../../types/verification-agent";
import { getDefaultVerificationConfig } from "../../types/verification-agent";

interface VerificationPanelProps {
  /** Callback when verification completes to show report */
  onViewReport?: (runId: string) => void;
}

export function VerificationPanel({ onViewReport }: VerificationPanelProps) {
  ...
  // Build config object
  const buildConfig = useCallback((): VerificationTaskConfig => {
    const base = getDefaultVerificationConfig(effectiveConfigPath);
    return {
      ...base,
      strategy: strategy as VerificationTaskConfig["strategy"],
  ...
  }, [buildConfig, startVerification, onViewReport, clearError]);

  // Check if we can run verification
  const canRunVerification = useMemo(() => {
    return loadedConfig !== null && !verificationRunning;
  }, [loadedConfig, verificationRunning]);
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-shrink-0 p-4 border-b border-border">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-semibold">AI Verification Agent</h2>
        </div>
      ...
      <div className="flex-1 overflow-auto p-4 space-y-4">
        ...
        <div className="p-3 bg-card rounded-lg border border-border">
          ...
          {!loadedConfig && (
            <p className="text-xs text-amber-500 mt-1">
              Load a configuration in the Run tab to enable verification
            </p>
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          <select
            ...
            disabled={strategiesLoading || verificationRunning}
      ...
      <div className="flex-shrink-0 p-4 border-t border-border flex gap-3">
        <button
          ...
          disabled={!canRunVerification || previewLoading}
 ...
 */
function PlanPreview({ plan }: { plan: VerificationPlan }) {
