...
export function SpecSourceSection({ onSpecsChanged }: SpecSourceSectionProps) {
  ...
  useEffect(() => {
    async function loadSpecs() {
      ...
      // Merge any additional specs from storage (e.g. from external apps)
      let persistedSelectedIds: string[] | null = null;
      let persistedPages: DiscoveredPage[] = [];
      let persistedPageUrls: string[] = [];
      const parsed = instanceStorage.getJSON<PersistedSpecState | null>(STORAGE_KEY, null);
      if (parsed) {
      ...
      let storedVersion = 0;
      const versionData = instanceStorage.getJSON<{ bundledSpecVersion?: number } | null>(
        STORAGE_KEY,
        null,
      );
      if (versionData) {
        storedVersion = versionData.bundledSpecVersion ?? 0;
      }
  ...
  useEffect(() => {
    ...
    instanceStorage.setJSON(STORAGE_KEY, toSave);
