 ...
 *
 * One round closed that class on four surfaces and left three siblings open
 * in the same `actions: [...]` array, while its own docstring said they were
 ...
 * `handler: guardedHandler(…)`, or a handler with no parameter to receive one.
 * The guard wraps the HANDLER so the registration stays a plain literal that
 * carries its `effect` — see `guardedHandler.ts` for why a def-level wrapper
 * was replaced. A surface
 ...
 * the right things. `guardedHandler.test.ts`,
 * `terminalLaunchMenuActions.test.ts` and `TerminalBridgeProxies.test.tsx`
 * execute guarded handlers against `5`, `"zz"`, `[]`, an undeclared key and a
 * non-scalar value, with the effect functions spied, and assert the effect
 * count is ZERO. Together: every surface routes
...
describe("UI Bridge action surfaces — the enumeration", () => {
  ...
  it("the shape rule has NO exemption, and the guard module binds", () => {
    // An exemption list is how an enforcement mechanism dies. The def-level
    // wrapper this replaced had to exempt its own module; the handler-level
    // guard builds no surface, so there is nothing to exempt — asserted, so a
    // regression to building `{id, handler}` in the guard module shows here.
    const guardFile = "src/lib/ui-bridge/guardedHandler.ts";
    expect(SURFACES.filter((s) => s.file === guardFile)).toEqual([]);
    // The guard is only a guard while it calls the binder.
    const guardSource = readFileSync(join(ROOT, guardFile), "utf8");
  ...
  it("every guarded surface also declares its `effect` safety class", () => {
    // The two halves of an action's contract, held together: what it
    // ACCEPTS (the guard) and what class of effect it DECLARES (the effect
    // calculus). A guarded surface is by construction one that takes
    // arguments, so it is exactly the kind an autonomous walk must be able to
    // classify before it sends any.
    const unclassified = SURFACES.filter((s) => s.form === "guarded" && s.effect === null).map(
      (s) => `${s.file}:${s.line} ${s.id ?? "(anonymous)"}`,
    );
...
describe("UI Bridge action surfaces — the falsification", () => {
  ...
  it(
    ...
    () => {
      ...
      // pass every assertion above. Written the way the app writes one: a plain
      // literal carrying `effect`, with the guard on the handler.
      const found = scanFixture(
        "guardedProbe.ts",
        [
          'import { guardedHandler } from "@/lib/ui-bridge/guardedHandler";',
          'const PROBE_SCHEMA = { count: "number" } as const;',
          "export const probe = {",
          "  actions: [",
          "    {",
          '      id: "probe-guarded",',
          ...
          '      effect: "read",',
          '      handler: guardedHandler("probe-guarded", PROBE_SCHEMA, (args) => args.count),',
          "    },",
          "  ],",
          "};",
        ].join("\n"),
      ...
      expect(found.map((s) => [s.form, s.id, s.effect])).toContainEqual([
        "guarded",
        "probe-guarded",
        "read",
      ]);
  ...
  it(
    ...
    () => {
      ...
      // contract and the guard enforces another.
      const found = scanFixture(
        "driftedGuard.ts",
        [
          'import { guardedHandler } from "@/lib/ui-bridge/guardedHandler";',
          'const PUBLISHED = { name: "string" } as const;',
          'const ENFORCED = { name: "string", extra: "string" } as const;',
          "export const probe = {",
          "  actions: [",
          "    {",
          '      id: "probe-drifted",',
          "      paramSchema: PUBLISHED,",
          '      effect: "write",',
          '      handler: guardedHandler("probe-drifted", ENFORCED, (args) => args),',
          "    },",
          "    {",
          '      id: "probe-unpublished",',
          '      effect: "write",',
          '      handler: guardedHandler("probe-unpublished", ENFORCED, (args) => args),',
          "    },",
          "  ],",
          "};",
        ].join("\n"),
      );
      expect(violations(found).map((v) => v.id)).toEqual(["probe-drifted", "probe-unpublished"]);
    },
    WALK_TIMEOUT,
  );
});
