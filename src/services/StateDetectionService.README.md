...
try {
  const result = await service.processSession(
    '/path/to/screenshots/session_123',
    {
      minRegionSize: [20, 20],
...
const topLeftElements = result.stateImages.filter(img =>
  img.x < 200 && img.y < 200
);
...
result.states.forEach(state => {
  const elements = result.stateImages.filter(img =>
    state.stateImageIds.includes(img.id)
  );
