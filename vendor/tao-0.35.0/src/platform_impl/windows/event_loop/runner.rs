...
// Arc, not Rc: `EventLoopWindowTarget` (which holds this) is reachable from
// `tauri_runtime_wry::DispatcherMainThreadContext`, which is `unsafe impl Send + Sync`.
// Cloning a `tauri::AppHandle`/`Webview` off the main thread therefore bumps this refcount
// concurrently with the main thread. A non-atomic `Rc` count races and corrupts, aborting in
// `inc_strong` (ud2 / 0xc000001d) or tripping the `rc.rs` `assert_unchecked` UB check.
// The pointee is still only *dereferenced* on the main thread; only the count needs to be atomic.
// Upstream: https://github.com/tauri-apps/tauri/issues/15408
